/*
 * NoDE is written by o6n <nick@incise.org> in 2005 & 2011.
 *
...
void kill_win(Window w){
	...
	event.xclient.data.l[0] = XInternAtom(dsp, "WM_DELETE_WINDOW", True);
	event.xclient.data.l[1] = CurrentTime;
	XSendEvent(dsp, w, False, NoEventMask, &event);
...
int main(void){
	...
	/*
	 * Super_L + F1 focus window
	 * Super_L + F2 launch xterm
	 * Super_L + F3 rofi drun
	 * Super_L + F4 close window
	 * Super_L + F12 exit window manager
	 * Super_L + Tab rofi window
	 * Super_L + Button1 left mouse move window
	 * Super_L + Button3 right mouse resize window
	 * Super_L + Button4 mouse scrollwheel up
	 * Super_L + Button5 mouse scrollwheel down
	 */
	...
	for(;;){
		...
		if(ev.type == KeyPress && ev.xkey.subwindow != None){
			...
			*/
			...
			/* Super_L + tasto sinistro mouse + click touchpad */
			/*if(ev.xbutton.button == Button1){*/
			/*if(start.button==1){
				XMoveResizeWindow(dsp, start.subwindow, 0, 0, screen->width / 2, screen->height);
			}*/
			/* Super_L + tasto destro mouse + click touchpad */
			/*if(start.button==3){
				XMoveResizeWindow(dsp, start.subwindow, screen->width / 2, 0, screen->width / 2, screen->height);
			}*/
			/* clarawm */
			/*
			if(ev.xbutton.y_root < 20){
				XMoveResizeWindow(dsp, start.subwindow, 0, 0, screen->width, screen->height / 2);
			}
			else if(ev.xbutton.y_root > screen->height - 20){
				XMoveResizeWindow(dsp, start.subwindow, 0, screen->height / 2, screen->width, screen->height / 2);
			}
			else if(ev.xbutton.x_root < 20){
				XMoveResizeWindow(dsp, start.subwindow, 0, 0, screen->width / 2, screen->height);
			}
			else if(ev.xbutton.x_root > screen->width - 20){
				XMoveResizeWindow(dsp, start.subwindow, screen->width / 2, 0, screen->width / 2, screen->height);
			}
			*/
			/*else{*/
